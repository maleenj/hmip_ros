...
class hand_tracker:
    ...
    def init_ros_components(self):
        ...
        self.velocity_noise = rospy.get_param('~velocity_noise', 0.1)
        ...
        if self.use_compressed:
            self.rawimg_sub = rospy.Subscriber('/usb_cam/image_raw/compressed', CompressedImage, self.imgsub_cb)
        else:
        ...
        # self.handimg_pub = rospy.Publisher('~image/compressed', CompressedImage, queue_size=10)
    ...
    def tracker_update(self, event):
        ...
        try:
            if self.use_compressed:
               frame = self.bridge.compressed_imgmsg_to_cv2(self.img_msg)
            else:   
        ...
        if results.multi_hand_landmarks:
            ...
            if np.abs(self.v[0]) < self.velocity_noise and np.abs(self.v[1]) and np.abs(self.v[2]) < self.velocity_noise:
                self.v=[0,0,0]
            else:
                self.wp2=self.wp1
        ...
        # self.detection_img=self.bridge.cv2_to_compressed_imgmsg(frame)
